...
let rec check_list input_list element =
	...
	(* First, I need to check if the list is empty, if it is, return false *)
	| [] -> false 
	| head_pos :: tail_pos ->
	(* Second, check if the head element equal to the search element, if yes, return true *)
...
let rec subset a b =
	match a with
	(* First, empty list is considered as subset of list b *)
	| [] -> true
	| head_pos :: tail_pos ->
	(* Second, we will check whether the head element of list a is inside the list b, if yes, then recursively search 
...
let rec set_union a b = 
	match a with 
	(* If a is a empty set, then union of set a and b is just set b *)
	| [] -> b
...
(* The purpose of the first case is to go around the reminding message *)
(* let rec set_intersection a b =
	match a with 
	| [] -> []
	| head_pos :: tail_pos -> if (check_list b head_pos)
	then head_pos :: (set_intersection tail_pos b)
	else (set_intersection tail_pos b);; *)
...
(* let rec set_diff a b =
	...
	| [] -> []
	| head_pos :: tail_pos -> if (check_list b head_pos)
	then (set_diff tail_pos b)
	else head_pos :: (set_diff tail_pos b);; *)
...
(* let rec extract_reachable current_symbols filter_rules = 
	if (equal_sets (extract_symbols current_symbols filter_rules) (extract_symbols (extract_symbols current_symbols filter_rules) filter_rules)) 
	then (extract_symbols current_symbols filter_rules)
	else extract_reachable (extract_symbols (extract_symbols current_symbols filter_rules) filter_rules) filter_rules;; *)
...
(* let rec remove_all_unreachable output_rules the_head =
	List.filter (fun elem -> check_list (extract_reachable [the_head] output_rules) (extract_left_tuple elem)) output_rules;; *)
...
(* let rec filter_reachable g =
	((extract_left_tuple g), remove_all_unreachable (extract_right_tuple g) (extract_left_tuple g));; *)
